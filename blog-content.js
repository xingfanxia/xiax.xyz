...
var BLOG_CONTENT = {
    ...
    "companion-zh": [
        ...
        "## 不是聊天机器人——是有机体",
        "",
        "这不是 NSFW 聊天机器人，不是女友/男友模拟器。这是一个**24小时全天候的人生教练**——你定义角色，它记住一切。不只是事实，还有事实背后的模式。",
        "",
        "你在为一个重大决定焦虑？它知道你在类似决定上的历史。它知道你容易过度思考。它知道上次你直接做了决定之后更开心。它告诉你这些，不是因为它被编程为\"有帮助的\"，而是因为它**了解你**。",
        "",
        "要做到这一点，AI伴侣不能是一个单一的模型。它需要一个**像器官一样运作的Agent系统**：记忆Agent提取人格信号，情绪Agent调整语气，判断Agent决定什么时候该反驳、什么时候该安慰，主动Agent捕捉该联系你的时机。它们协调运作——记忆Agent持续向其他Agent输送信号，判断Agent可以推翻默认回应，情绪Agent在发送前微调每条回复。整个系统在每次互动前汇聚：我们知道什么，这个人现在需要什么，最好的传达方式是什么？",
        "",
        "你不是在构建聊天机器人——你是在构建一个**有机体**。",
        "",
        "这正是我在用 [OpenClaw](https://github.com/xingfanxia/openclaw) 做原型的方向。SOUL.md 定义人格——追求、缺陷、情绪范围。HEARTBEAT.md 控制自主性——Agent根据对话模式和情绪状态评估是否该主动联系你。MEMORY.md 处理跨对话连续性——提取人格信号，存储为未来会话的上下文。",
        "",
        "早期结果很有意思：当AI有了连续性和性格，工具和实体之间的界限很快就变得模糊了。",
        "",
        "## 原型其实已经建好了",
        "",
    ...
    "lastmile-zh": [
        ...
        "## 认知鸿沟",
        "",
        "我朋友用过 ChatGPT，用过豆包，用过各种聊天机器人。他对 AI 的认知和大多数人一样：一个比搜索引擎聪明一点的问答工具。这大概是 99.99% 的人的理解。",
        "",
        "他比大多数人多一个条件——我平时会和他分享 AI Agent 的真实使用场景，他知道\"应该很厉害\"。但知道和亲眼看到是两回事。",
        "",
        "那个周末我们做了三件事。Claude Code 读取他的财务数据，建模，生成分析报告，根据反馈迭代——不是回答关于财务的问题，是真正在做财务分析。然后我们从零做了一个用声音音调控制的 Flappy Bird——一个多小时，从零到能玩，两个成年人对着电脑嚎叫，试图控制那只鸟不撞管子。最后 Claude Code 帮他搭了一个完全针对他业务场景的定制工具——不是通用软件的配置，是一个只为他的需求而存在的应用。",
        "",
        "三件事做完，他说了一句话：**\"一个小时前，这些东西都不存在。\"**",
        "",
    ...
    "claudecode-zh": [
        ...
        "",
        "Claude Code 做的事情完全不同。它在模型外面套了一层**缰绳**（harness）——读取代码库、制定执行计划、调用工具、执行命令、验证结果、自行调试、迭代直到任务完成。缰绳把模型的行动空间从\"回答问题\"拉到了\"完成任务\"。",
        "",
        ...
        "",
        "但这个故事只讲了一半。缰绳很重要——但更重要的是**大脑本身在进化**。",
        "",
        "今天的 foundation model 不再是一个文本补全机器。Opus 能自主思考下一步该做什么，判断该调用哪个工具，根据返回结果决定下一步行动，连续做出一系列决策来完成一个复杂任务。这种\"思考→行动→判断→再行动\"的循环，就是 agentic model 的本质。关键不只是它多了手脚去调用工具——而是它有了**调度工具和做出判断**的能力。缰绳给了这个大脑足够多的工具和双手。代码库访问、shell 命令、API 调用、文件系统——行动空间。但真正做决策的，是模型自己。",
        "",
        "这和以前所有的自动化完全不同。传统的非 agentic 系统——CI/CD 管线、自动化脚本、RPA——按照写死的规则执行：if A then B, else C。没有判断力，遇到意外就卡死。它们有手脚，但没有大脑。",
        "",
        "Agentic 范式反过来：**大脑在前，手脚在后。** 模型先理解任务、拆解目标、规划路径，然后调用工具执行。遇到障碍不会卡死——它换一条路。这就是为什么 agentic 范式能成为真正的通用 Agent：能力边界不再是预设的规则，而是模型的判断力。而模型的判断力，在以指数级增长。",
        "",
        ...
        "",
        "Claude Code 还解锁了一个独特的扩展维度：并行 Agent。简单的 bug 用一个 Agent 就够了。复杂的问题？Boris 会同时启动三到五个甚至十个 Agent，每个从不同角度调查。他称之为\"不相关的上下文窗口\"（uncorrelated context windows）——更多独立的上下文窗口意味着更强的整体能力，不仅仅是更快。这是并行思考，不是并行计算。",
        "",
        ...
        "",
        "甚至招聘方式都在跟着变。YC 正在尝试让工程候选人提交 Claude Code 会话记录——他们和 Agent 一起开发功能的完整过程。\"你能看出一个人怎么思考，\"YC 合伙人说，\"他们会不会看日志？Agent 跑偏了能不能纠正？有没有系统思维？\"问题不再是\"你会不会写代码\"——而是\"你能不能驾驭 Agent\"。",
        "",
